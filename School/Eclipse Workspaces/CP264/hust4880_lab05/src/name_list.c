...
void insert_name_list(sNAME_LIST *name_list, sNODE *node) {
	...
	if (name_list->count <= 0) {
		...
		while (strcmp(node->name.first_name, current->name.first_name) != 0
				&& strcmp(node->name.last_name, current->name.last_name) != 0
				&& current->next != NULL) {
			current = current->next;
		}

		if (strcmp(node->name.first_name, current->name.first_name) == 0
				&& strcmp(node->name.last_name, current->name.last_name) == 0) {
			free(node);
		} else {
