...
static bst_node *bst_delete_node(bst_node *parent) {
	...
	if(repl_node->value < parent->value){
		repl_node->height = parent->left->height;
		repl_node->height = parent->left->left;
		repl_node->height = parent->left->right;
		repl_node->height = parent->left->value;
		free(parent->left);
		parent->left = repl_node;
	} else if(repl_node->value > parent->value){
		repl_node->height = parent->right->height;
		repl_node->height = parent->right->left;
		repl_node->height = parent->right->right;
		repl_node->height = parent->right->value;
		free(parent->right);
...
//=================================
{
	...
	while(node->value != key && node != NULL){
		if(key < node->value){
			node = node->left;
