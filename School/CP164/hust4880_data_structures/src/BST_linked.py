...
class BST:
    ...
    def __contains__(self, key):
        ...
        contains = self.contains_aux(self._root, key)
        
        return contains
    
    def contains_aux(self, parent, key):
        contains = False
        if(self._count > 0):
            if(key == parent._value):
                contains = True
            else:
                if(key < parent._value and parent._left != None):
                    contains = self.contains_aux(parent._left, key)
                elif(key > parent._value and parent._right != None):
                    contains = self.contains_aux(parent._right, key)
        return contains
    ...
    def is_identical(self, other):
        ...
        -------------------------------------------------------
        """
        if(self._root != None and other._root != None or self._root == None and other._root == None):
            identical = self.is_identical_aux(self._root, other._root)
        else:
            identical = False
        return identical

    def is_identical_aux(self, s_node, o_node):
        if(s_node == None and o_node == None):
            identical = True
        
        elif(s_node != None and s_node != None and s_node._value == o_node._value and s_node._height == o_node._height):
            identical = self.is_identical_aux(s_node._left, o_node._left) and self.is_identical_aux(s_node._right, o_node._right)
    ...
    def parent_r(self, key):
        ...
        value = self.parent_aux(node, parent, key)
        
        return value
    
    def parent_aux(self, node, parent, key):
        value = None
        ...
        elif(key < node._value and node._left != None):
            parent = node
            node = node._left
            value = self.parent_aux(node, parent, key)
            
        elif(key > node._value and node._right != None):
            parent = node
            node = node._right
            value = self.parent_aux(node, parent, key)
    ...
    def one_child_count_aux(self, node):
        count = 0
        if(node._left != None and node._right == None or node._left == None and node._right != None):
            count = 1
    ...
    def is_balanced(self):
        ...
        """
        if(not self.is_empty()):
            balanced = self.is_balanced_aux(self._root._left, self._root._right)
        else:
            balanced = True
        return balanced
        
        
    def is_balanced_aux(self, Left, Right):
        
        balanced = True
        
        #Goes through nodes on the left
        if(Left != None and Right != None and abs(Left._height - Right._height) <= 1):
            balanced = self.is_balanced_aux(Left._left, Left._right)
        ...
        #Right nodes traversed as it goes back to base
        if(balanced and Left != None and Right != None):
            balanced = self.is_balanced_aux(Left._left, Left._right)
    ...
    def is_valid(self):
        ...
        """
        if(not self.is_empty()):
            valid = self.is_valid_aux(self._root)
        else:
            valid = True
        return valid
    
    def is_valid_aux(self, node):

        #Goes through nodes on the left
        if(node._left != None and node._right != None and node._left._value <= node._value and node._right._value >= node._value):
            valid = self.is_valid_aux(node._left)
            
        elif(node._left != None and node._right == None and node._left._value <= node._value):
            valid = True
        
        elif(node._left == None and node._right != None and node._right._value >= node._value):
            valid = True
        ...
        #Right nodes traversed as it goes back to base
        if(valid and node._left != None and node._right != None):
            valid = self.is_valid_aux(node._right)
    ...
    def preorder(self):
        ...
        a = []
        self.preorder_aux(self._root, a)
        return a
    ...
    def levelorder(self):
        ...
        if(self._root != None):
            bst = []
            bst.append(self._root)
            while(bst != []):
                node = bst.pop(0)
                values.append(node._value)
                if(node._left != None):
                    bst.append(node._left)
                if(node._right != None):
                    bst.append(node._right)
        return values   
