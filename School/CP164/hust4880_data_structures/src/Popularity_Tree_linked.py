...
class Popularity_Tree:
    ...
    def _retrieve_aux(self, node, key):
        ...
        if(node._value == key):
            ...
            if(node._left._value != None and node._left._value == key):
                #key found
                value = deepcopy(node._left._value)
                node._left._rcount += 1
                #rotate node right if necessary
                if(node._rcount < node._left._rcount):
                    node = self._rotate_right(node)
        
            elif(node._left._value != None and node._left._value != key):
                value = self._retrieve_aux(node._left, key)
        else:
            if(node._right._value != None and node._right._value == key):
                #key found
                node._right._rcount += 1
                value = deepcopy(node._right._value)
                #rotate node left if necessary
                if(node._rcount < node._right._rcount):
                    node = self._rotate_left(node)
                elif(node._right._value != None and node._right._value != key):
                    value = self._retrieve_aux(node._left, key)
        return node, value
        '''
        if(node == None):
            ...
            if(node._value == key):
                ...
                parent, value = self._retrieve_aux(node._left, key)  
                if(parent != None and node._rcount < parent._rcount):
                    node = self._rotate_right(node)
                else:
                    parent = node  
                
            else:
                parent, value = self._retrieve_aux(node._right, key)  
                if(parent != None and node._rcount < parent._rcount):
                    node = self._rotate_right(node)
                else:
                    parent = node  
    ...
    def _rotate_left(self, parent):
        ...
        temp_pivot = deepcopy(parent._right)
        
        updated = parent
        
        updated._left = deepcopy(parent)
        updated._left._height -= 1
        
        updated._right = temp_pivot._right
        
        updated._left._right = temp_pivot._left
        
        updated._value = temp_pivot._value
        ...
        old_left = updated._left
        
        updated._left = parent
        
        parent._right = old_left
    ...
    def _rotate_right(self, parent):
        ...
        old_right = updated._right
        
        updated._right = parent
        
        parent._left = old_right
    ...
    def _is_valid_aux(self, node):
        ...
        """
        print(node._rcount)
        #print(node._height, node._value)
        if(node._left != None and node._left._rcount <= node._rcount):
            valid = self._is_valid_aux(node._left)
        elif(node._left != None and node._left._rcount > node._rcount):
            valid = False
        elif(node._left == None):
            valid = True
        
        if(node._right != None and node._right._rcount <= node._rcount):
            valid = self._is_valid_aux(node._right)
        elif(node._right != None and node._right._rcount > node._rcount):
            valid = False
